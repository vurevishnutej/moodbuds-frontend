...
export const productAdminApi = {
  async listMoods(): Promise<AdminMoodOption[]> {
    const res = await adminApiClient.get<PageResponse<MoodRow>>('/admin/moods?page=0&size=100');
    return res.content
      .filter((m) => toBool(m.is_active))
  ...
  async listProducts(query = ''): Promise<AdminProductRow[]> {
    ...
    const res = await adminApiClient.get<PageResponse<ProductSummaryRow>>(`/admin/products/summary?page=0&size=200${q}`);
    return res.content.map((r) => {
      ...
      return {
        ...
        stock: r.stock,
        status,
        image: r.primaryMediaId != null ? mediaImageUrl(r.primaryMediaId) : null,
        moodName: r.moodName,
  ...
  async getProduct(id: number): Promise<AdminProduct> {
    ...
    return {
      ...
      image: res.images && res.images.length > 0 ? mediaImageUrl(res.images[0].mediaId) : '/placeholder.png',
      mrp: res.product.discountPrice ? Number(res.product.price) : undefined,
      ...
      images: res.images ? res.images.map((img: any) => ({ mediaId: img.mediaId, url: mediaImageUrl(img.mediaId) })) : [],
  ...
  async createProduct(payload: CreateProductPayload): Promise<{ id: number }> {
    const body = {
      ...
      images: payload.imageMediaIds.map((mediaId, index) => ({
        mediaId,
        primary: index === 0,
        sortOrder: index,
      })),
