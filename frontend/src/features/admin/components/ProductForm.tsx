...
function toFormValue(p?: AdminProduct): ProductFormValue {
  ...
  return {
    ...
    sizes: p?.sizes ?? [],
    sizeStocks,
    images: p?.images && Array.isArray(p.images) ? p.images.map((img: any) => 
      typeof img === 'string' ? { mediaId: 0, url: img } : { mediaId: img.mediaId || 0, url: img.url || '' }
    ) : [],
...
export function ProductForm({
  ...
  const removeImage = (mediaId: number) =>
    setValue((prev) => ({ ...prev, images: prev.images.filter((img) => img.mediaId !== mediaId) }));
              ...
              {value.images.length > 0 && (
                <div className="adm-img-strip">
                  {value.images.map((img, i) => (
                    <div className="adm-img-thumb" key={img.mediaId}>
                      <img src={img.url} alt="" />
                      <button type="button" className="adm-img-remove" onClick={() => removeImage(img.mediaId)} aria-label="Remove image">✕</button>
